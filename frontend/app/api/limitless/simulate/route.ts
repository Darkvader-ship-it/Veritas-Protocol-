...
/**
 * POST /api/limitless/simulate
 * Place a simulated bet on a Limitless market
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      priceAtEntry, // Price (0-1) at time of bet
      expiresAt,
    } = body;
    ...
    // Check for existing bet on this market
    ...
    // Calculate potential payout (1 / price)
    ...
    if (error) {
      console.error('Error inserting Limitless trade:', error);
      ...
      if (error.code === '42P01' || error.code === 'PGRST205' || String(error.message).includes('does not exist')) {
        return NextResponse.json({
          ...
          message: `Real ${amount} tUSDC via Somnia vault`,
        });
      }
    ...
    return NextResponse.json({
      ...
      message: `Simulated $${amountNum} on ${position} at ${(parseFloat(priceAtEntry) * 100).toFixed(0)}%`,
    });
  } catch (error: any) {
    console.error('Limitless simulate error:', error);
    return NextResponse.json(
...
/**
 * GET /api/limitless/simulate
 * Get simulated Limitless trades for a user
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('Limitless simulate GET error:', error);

    if (error.code === '42P01' || error.code === 'PGRST205' || String(error.message).includes('does not exist')) {
      return NextResponse.json({
        trades: [],
        count: 0,
        warning: 'Limitless simulation table not configured',
      });
    }
