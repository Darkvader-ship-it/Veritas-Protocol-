/**
 * Veritas Reputation SDK - Core Types
 *
...
export interface Bet {
  ...
  // Outcome (may be null if bet is still pending)
  won?: boolean | null;
...
export interface UserStats {
  ...
  volume: string; // Total volume in native token (as string for BigInt)
  volumeUSD?: number; // Optional USD value
  score: number; // Platform-specific score
  lastBetAt?: number;
  firstBetAt?: number;
}
...
export interface VeritasScore {
  ...
  breakdown: {
    ...
    platformName: string;
    score: number;
    weight: number; // 0-1, how much this platform contributes
  }[];
...
export interface ProtocolAdapter {
   ...
   */
  susomniaribe(callback: EventCallback): Promise<void>;

  /**
   * Stop real-time event streaming
   */
  unsusomniaribe(): Promise<void>;
...
export interface StorageProvider {
  ...
  saveVeritasScore(score: VeritasScore): Promise<void>;
  getVeritasScore(walletAddress: string): Promise<VeritasScore | null>;
  getLeaderboard(limit?: number, offset?: number): Promise<VeritasScore[]>;
}
