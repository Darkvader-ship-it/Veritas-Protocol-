...
const client = createPublicClient({
  chain: somnia,
  transport: http('https://somnia-dataseed1.somnia.org'),
});

/**
 * Calculate VeritasScore based on performance
 * Formula: (winRate * 100) + (totalBets * 2) + (volume in STT / 10)
 */
function calculateVeritasScore(stats: UserStats): number {
  ...
  const score = Math.floor(
    ...
    (volumeInSTT / 10) // Volume contribution
  );
...
/**
 * Fetch all bet events from DreamDEXPrediction contract
 */
  ...
  for (let start = fromBlock; start <= toBlock; start += batchSize) {
    ...
    try {
      ...
      const bullBets = await client.getLogs({
        address: DREAMDEX_PREDICTION_ADDRESS,
...
function processUserStats(bullBets: any[], bearBets: any[], claims: any[]): Map<Address, UserStats> {
  ...
  for (const [address, user] of userMap) {
    ...
    user.veritasScore = calculateVeritasScore(user);
  }
...
function printSummary(data: RealUsersData) {
  ...
  data.users.slice(0, 10).forEach((user, index) => {
    ...
    console.log(`   Score: ${user.veritasScore} | Bets: ${user.totalBets} | Win Rate: ${user.winRate.toFixed(1)}%`);
    console.log(`   Volume: ${(Number(user.totalVolume) / 1e18).toFixed(4)} STT\n`);
  });
