...
The VeritasScore is Veritas's reputation metric that aggregates prediction market performance into a single, verifiable number.

## Overview

VeritasScore balances two key factors:
...
|-----------|-------------|---------|
| Win Rate | `wins / totalBets × 10000` | Basis points (0-10000) |
| Volume Multiplier | `sqrt(totalVolume / 1e16)` | Square root scaling |
| Final Score | `winRate × volumeSqrt / 100` | Combined metric |
...
Wins: 15
Total Bets: 25
Volume: 2 STT (2e18 wei)

Win Rate: 15/25 = 60% = 6000 bps
Volume Sqrt: sqrt(2e18 / 1e16) = sqrt(200) ≈ 14.14

VeritasScore = (6000 × 14.14) / 100 = 848
```
...
Volume Sqrt: sqrt(45.5e18 / 1e16) ≈ 67.45

VeritasScore = (6333 × 67.45) / 100 = 4272
```
...
Volume Sqrt: sqrt(200e18 / 1e16) ≈ 141.4

VeritasScore = (5200 × 141.4) / 100 = 7353
```
...
|------|-------------|----------|
| 🥉 Bronze | 0 - 499 | Basic profile |
| 🥈 Silver | 500 - 999 | Leaderboard visibility |
| 🥇 Gold | 1000 - 1999 | Copy trading eligible |
| 💎 Platinum | 2000 - 4999 | Priority visibility |
| 👑 Diamond | 5000+ | Premium status |
...
```
User Import → VeritasCore.importPredictions()
                     │
                     ▼
...
<Tabs items={['On-Chain', 'Off-Chain']}>
  <Tabs.Tab>
    ...
    uint256 score = veritasCore.getUserScore(userAddress);
  ...
  <Tabs.Tab>
    ...
    const { veritasScore, tier, rank } = await response.json();
...
| VeritasScore | winRate × sqrt(volume) | Balanced | Novel |
...
- [VeritasCore](/contracts/veritas-core)
