...
- Deposit STT into a managed vault
...
│  Follower Balances                                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐             │
│  │ User A: 5 STT│ │ User B: 10 STT│ │ User C: 2 STT│            │
│  └─────────────┘  └─────────────┘  └─────────────┘             │
├─────────────────────────────────────────────────────────────────┤
│  Follow Relationships                                           │
│  User A → Leader X (50% allocation, 0.5 STT max)               │
│  User A → Leader Y (30% allocation, 0.3 STT max)               │
│  User B → Leader X (100% allocation, 1 STT max)                │
├─────────────────────────────────────────────────────────────────┤
│  When Leader X bets 1 STT:                                      │
│  → User A copies 0.5 STT (50% of 1, capped at max)             │
│  → User B copies 1 STT (100% of 1, at max)                     │
└─────────────────────────────────────────────────────────────────┘
```
...
```
Scenario: Follower copies 0.5 STT bet, wins 0.45 STT profit

Gross Payout:     0.95 STT (0.5 original + 0.45 profit)
Protocol Fee:     0.045 STT (10% of 0.45)
Leader Fee:       0.045 STT (10% of 0.45)
Net to Follower:  0.86 STT (0.5 original + 0.36 profit)
...
function executeCopyTrade(
    ...
    if (isBull) {
        dreamDEX.betBull{value: copyAmount}(epoch);
    } else {
        dreamDEX.betBear{value: copyAmount}(epoch);
    }
...
function CopyTradingPanel() {
  ...
  return (
    <div>
      <p>Balance: {formatEther(balance)} STT</p>
